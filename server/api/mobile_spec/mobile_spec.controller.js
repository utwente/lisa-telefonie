    ...
    TMobile.findOne({month: req.params.month}, function (err, t_mobile) {
        ...
        c.load(function(err) {
            ...
            for (var i = t_mobile.length - 1; i >= 0; i--) {
                ...
                data = c.findDepartmentByNumber(t_mobile[i].number)

                switch (data.code) {
                    case 'NO_CUSTOMER_DATA':    return res.status(404).send(data); break;   // there are no customers defined, serious error, stop all execution.
                    case 'NO_DEPARTMENT_DATA':  return res.status(404).send(data); break;   // there are no departments defined, serious error, stop all execution.
            ...
            */
            var filename = _.template(yearMonth + ' specificatie categorie <%= department %>.pdf');
            var title = _.template('T-mobile specificatie totalen per categorie <%= department %>');
            ...
            for (var department in department_spec) {
                ...
                pdf.generate(fullPath + filename({department: department}), title({department: department}), data, totalPDF);
            ...
            */
            var filename = _.template(yearMonth + ' specificatie nummers <%= department %>.pdf');
            var title = _.template('T-mobile specificatie totalen individueel <%= department %>');
            ...
            */
            ...
            var filename = _.template(yearMonth + ' specificatie personen <%= department %>.xlsx');
            ...
            for (var department in department_spec) {
                ...
                xslx.generate(fullPath + filename({department: department}), data, totalExcel);
            ...
            */
            ...
            var filename = _.template(yearMonth + ' specificatie nummers <%= department %> <%= number %>.html');
            var costs;
            var total;
            var user;
            for (var department in department_spec) {
                for (var i = 0; i < department_spec[department].length; i++) {
                    ...
                    if (total > 0) {
                        ...
                        user.number = number;
                        html.generate(fullPath + filename({department: department, number: number}), user, data, total, totalHTML);
                        attachments.departments[department].html.push({
                            number: number, 
                            path: fullPath,
                            filename: filename({department: department, number: number}),
                            send: false
                        });
                    }
                }
